...
public class FormatterNode implements GraphNode {
    ...
    public GraphState process(GraphState state) {
        log.info("[Manual Info][FormatterNode] Formatting response. Verified={}",
                state.getVerificationResult());

        String finalResponse;
        GraphState.VerificationResult vr = state.getVerificationResult();
        if (vr == null
                || vr == GraphState.VerificationResult.APPROVED
                || vr == GraphState.VerificationResult.SKIPPED) {
            // Verification disabled (SKIPPED, the field's default value), or
            // never set (null), or genuinely approved — all three mean "no
            // concerns to show the user", so render the plain answer.
        ...
        log.info("[Manual Info][FormatterNode] Final response ready ({} chars)",
                finalResponse.length());
        return state;
    }

    /**
     * Shows the primary LLM answer plus a highlighted panel explaining what the
     * verifier found different/missing.
     */
    private String buildResponseWithDifference(GraphState state) {
        ...
        html.append("<div style='")
                ...
                .append("'>")
                .append("<i class='fa-solid fa-clipboard'></i> Best available answer:")
                .append("</div>");
